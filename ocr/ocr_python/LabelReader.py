...
class LabelReader:
    ...
    def read_label(self, wsi_name):
        if self.os == "linux":
            # from lib.tslide.tslide import TSlide
            ...
            # slide.close()
            # return image
            lib = cdll.LoadLibrary('tslide/libkfbslide.so')
            
        else:
            lib = WinDLL("./lib/ImageOperationLib.dll")

            lib.GetLableInfoPathFunc.argtypes = [c_char_p, POINTER(POINTER(c_ubyte)), POINTER(c_int), POINTER(c_int), POINTER(c_int)]
            lib.GetLableInfoPathFunc.restype = c_int
            ...
            height = c_int()
            res = c_int()
            res = lib.GetLableInfoPathFunc(path, byref(imageData), byref(length), byref(width), byref(height))
            if res:
