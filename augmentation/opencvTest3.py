...
if __name__ == '__main__':
    ...
    # # for use of multiprocessing.Process
    # num = len(file_path_out)
    # p1 = Process(target=bath_augment, args=(file_list[:int(num/3)], size, file_path_out))
    # p2 = Process(target=bath_augment, args=(file_list[int(num/3):int(2*num/3)], size, file_path_out))
    # p3 = Process(target=bath_augment, args=(file_list[int(2*num/3):], size, file_path_out))
    # p1.start()
    # p2.start()
    # p3.start()
    ...
    pool.map(partial(single_augment, file_path_out=file_path_out, size=size), file_list)
