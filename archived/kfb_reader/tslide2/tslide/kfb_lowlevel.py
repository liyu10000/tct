...
try:
    ...
    _lib = cdll.LoadLibrary('tslide/libkfbslide.so')
...
def kfbslide_read_region(osr, level, pos_x, pos_y):
    ...
    return PIL.Image.open(io.BytesIO(np.ctypeslib.as_array(pixel, shape=(data_length.value,))))
...
def kfbslide_read_roi_region(osr, level, pos_x, pos_y, width, height):
    ...
    # return PIL.Image.frombuffer('RGBA', (width, height), pixel, 'raw', 'RGBA', 0, 1)
    return PIL.Image.open(io.BytesIO(np.ctypeslib.as_array(pixel, shape=(data_length.value,))))
