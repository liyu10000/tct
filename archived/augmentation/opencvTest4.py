...
class Img:
    ...
    def augment(self, w, h):
        ...
        self.translate(self.w * uniform(-0.2, 0.2), 0)  # 随机水平平移
        self.translate(self.w * uniform(-0.2, 0.2), 0)  # 随机水平平移
        self.translate(0, self.h * uniform(-0.2, 0.2))  # 随机垂直平移
        self.translate(0, self.h * uniform(-0.2, 0.2))  # 随机垂直平移
...
def run_single(file, file_path_out, size):
...
def run_batch(file_path_in, postfix, file_path_out, size):
    file_list = scan_files(file_path_in, postfix=postfix)

    if not os.path.exists(file_path_out):
        os.makedirs(file_path_out)

    for file in file_list:
        run_single(file, file_path_out, size)
...
if __name__ == '__main__':
    ...
    # file_path_in = "D:\\1002cellsFixedSize-0326\\#000000"
    # file_list = scan_files(file_path_in, postfix=".jpeg")
    #
    # file_path_out = "D:\\1002cellsFixedSize-0326\\#000000-augmented"
    # if not os.path.exists(file_path_out):
    #     os.makedirs(file_path_out)
    #
    # size = 512
    ...
    # # used to augment images in #000000 alike sub folders
    # for key in colors:
    #     file_path_in = "D:\\1002cellsFixedSize-0326\\" + key
    #     file_path_out = file_path_in + "-augmented"
    #
    #     run_batch(file_path_in, ".jpeg", file_path_out, 512)

    # used to augment images in a single folder
    ...
    pool.map(partial(run_single, file_path_out=file_path_out, size=size), file_list)
